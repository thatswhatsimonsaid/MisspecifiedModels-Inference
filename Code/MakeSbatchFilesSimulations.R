...
ParameterVector <- read.csv(paste0(dir,"/data/Linear/Parameters/ParameterVector_50_200.csv"))
...
for (i in 1:nrow(ParameterVector)) {
  ...
  sbatch_file <- file(paste0(dir,"/Code/Slurm/Linear/N50_200/",job_name, ".sbatch"), "w")
  writeLines(
    c(
      "#!/bin/bash",
