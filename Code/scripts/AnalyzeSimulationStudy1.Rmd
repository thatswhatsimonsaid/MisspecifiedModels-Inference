---
title: "Stats 572 - Simulation Study Comparison"
author: "Simon Dovan Nguyen"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
    toc_float: true
    toc_collapsed: true
    theme: lumen
    css: assets/styles.css
---

# Set Up

## Libr.
```{r}
library(stats)
library(tidyverse)
library(StatMatch)
library(xtable)
library(lmtest)
library(sandwich)
```

## Rm. Var.
```{r}
rm(list=ls())
dir = "/Users/simondn/Documents/Stats572/"
```

## Functions
```{r}
source(paste0(dir,"Code/functions/AIZResults.R"))
source(paste0(dir,"Code/functions/DiscrepancyFunction.R"))
source(paste0(dir,"Code/functions/MonteCarloResults.R"))
source(paste0(dir,"Code/functions/SimulationReformatResultsFunction.R"))
```

## Results
```{r}
### Set Up ###
AIZ = AIZResults(type = "Linear")
SimulationResultsList = readRDS(file = paste0(dir,"data/SimulationCases/Linear/SimulationResults.rds"))
SimulationResultsReformatted = SimulationReformatResultsFunction(SimulationResultsList)
SimSEMedianList = SimulationResultsReformatted$SimSEMedianList 
MonteCarloREsults = MonteCarloResults(SimulationResultsReformatted, 5)
RunTimeList = SimulationResultsReformatted$RunTimeList
CoverageList = data.frame(SimulationResultsReformatted$CoverageList)
ParameterVector = SimulationResultsReformatted$ParameterVector
```

# Comparison
```{r}
DiscrepancyResults = DiscrepancyFunction(epsilon = 0.1, AIZ, SimSEMedianList, CoverageList)
Dicrepancy = DiscrepancyResults$Discrepancy
LargeDiscrepancyIndices = DiscrepancyResults$LargeDiscrepancyIndices
DiscrepancyMeans = DiscrepancyResults$MeanDiscrepancy
LargeDiscrepancyIndices
DiscrepancyMeans
```
# xtable
```{r}
### AIZ Coverage and SE Table ###
# cbind(SimSEMedianList[,c(1:5)],CoverageList,SimSEMedianList[,c(6,7,8)]) %>%
# xtable(digits = 5,
#        caption = "Simulation Results (Linear Model)")

# ### Monte Carlo Results ###
# MonteCarloREsults$MonteCarliResults_Population %>%
#   xtable(digits = 5,
#          caption = "Monte Carlo Simulation $\theta_{Cond}$ Results (Linear Model)")
# 
# MonteCarloREsults$MonteCarliResults_Conditional %>%
#   xtable(digits = 5,
#          caption = "Monte Carlo Simulation $\theta_{Pop}$ Results (Linear Model)")
```

# Evaluate
```{r}
head(SimSEMedianList)
```

### Misspecificaition
```{r}
# Models with misspecification and more variables
SimSEMedianList %>% filter(SS == 50) %>% filter(K == 1)  %>% select(Difference) %>% colMeans() %>% as.numeric()
SimSEMedianList %>% filter(SS == 200) %>% filter(K == 1)  %>% select(Difference) %>% colMeans() %>% as.numeric()
```

```{r}
SimSEMedianList %>% filter(SS == 50) %>% filter(K == 5)  %>% select(Difference) %>% colMeans() %>% as.numeric()
SimSEMedianList %>% filter(SS == 200) %>% filter(K == 5)  %>% select(Difference) %>% colMeans() %>% as.numeric()
```
### Homo
```{r}
SimSEMedianList %>% filter(Homo. == 0) %>% filter(K == 1)  %>% select(Difference) %>% colMeans() %>% as.numeric()
SimSEMedianList %>% filter(Homo. == 0.5) %>% filter(K == 1)  %>% select(Difference) %>% colMeans() %>% as.numeric()
```
```{r}
SimSEMedianList %>% filter(Homo. == 0) %>% filter(K == 5)  %>% select(Difference) %>% colMeans() %>% as.numeric()
SimSEMedianList %>% filter(Homo. == 0.5) %>% filter(K == 5)  %>% select(Difference) %>% colMeans() %>% as.numeric()
```







