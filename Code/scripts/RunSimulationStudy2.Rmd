...
### Run Simulation ###
# UpToNum = 2
# ParameterVector = ParameterVector[1:UpToNum,]
NSim = 1000
SimulationResultsList = list(length = nrow(ParameterVector))
for(i in 1:nrow(ParameterVector)){
  print(paste0("Case ",i))
SimulationResultsList[[i]] = SimulationFunction(NSim = NSim, ParameterVector,i, type = "Logistic")
  write.csv(SimulationResultsList[[i]]$SimulationSEResults, file = paste0(dir, "data/SimulationCases/Logistic/StandardErrors/Case",i,".csv"))
  write.csv(SimulationResultsList[[i]]$SimCoverageFrequency, file = paste0(dir, "data/SimulationCases/Logistic/Coverage/Case",i,".csv"))
}
...
```{r}
DiscrepancyResults$MeanDiscrepancy
```
