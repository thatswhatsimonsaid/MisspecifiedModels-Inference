...
### My Results ###
file_list = list.files(path =  paste0(dir,"Results/Logistic/N50_200"),
                        pattern = "\\.rds$",
                        full.names = TRUE)
...
```{r}
SmallSampleResultsTable[,1:11] %>% xtable(digits = 5)
```


```{r}
cbind(AIZ[,1:5], (AIZ[,6:11] - SmallSampleResultsTable[,6:11]) %>% xtable(digits = 5)
```
